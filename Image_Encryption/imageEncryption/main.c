...
void pause(int, double, int*, SDL_Surface*, SDL_Surface*, int, int, int*, SDL_PixelFormat*); // garder la fenetre ouverte jusqu'a fermeture via l'event "click de souris sur la croix rouge"
...
/******/
    pause(L, R, tabBits, ecran, image, largeurImg, longueurImg, K,fmt);
...
/*******************************************************************/
void pause(int L, double R, int* tabBits, SDL_Surface* ecran, SDL_Surface* image, int largeurImg, int longueurImg, int* K, SDL_PixelFormat* fmt)
{
    int continuer = 1, i = 0;
    SDL_Event event;
            ...
            case SDL_KEYDOWN:
                switch(event.key.keysym.sym)
                {
                    ...
                    case SDLK_c: // presser la touche C : algo de chiffrement
                        ...
                        nomNewImage = "images/imageChiffree.bmp";
                        //printf("R : %f\n", R);
                        //for()
                        break;
