...
void pause(int, double, int*, SDL_Surface*, SDL_Surface*, int, int, int*, SDL_Rect); // garder la fenetre ouverte jusqu'a fermeture via l'event "click de souris sur la croix rouge"
Uint32 obtenirPixel(SDL_Surface*, int, int); // recuperer un pixel de format 32 bits
int* recupTabBitsRGB(SDL_Surface*, int, int, int, int*); // recuperer les bits de l'image RGB
...
void definirPixel(SDL_Surface*, int, int, Uint32); // changer un pixel de l'image
void retourImage(SDL_Surface*, SDL_Surface*, int*, int, int, int); // Passer du tab de bits à l'image
...
int main(int argc, char *argv[])
{
    ...
    positionImg.x = 0;
    positionImg.y = 0;
    int largeurImg = 256; // 800 - 512 - 256 - 100
    int longueurImg = 256; // 600 - 512 - 256 - 125
    ...
    image = IMG_Load("images/barbaraA.bmp");
    SDL_BlitSurface(image, NULL, ecran, &positionImg);
    SDL_Flip(ecran);
/******/

/******/
    pause(L, R, tabBits, ecran, image, largeurImg, longueurImg, K, positionImg);
...
/*******************************************************************/
void pause(int L, double R, int* tabBits, SDL_Surface* ecran, SDL_Surface* image, int largeurImg, int longueurImg, int* K, SDL_Rect positionImg)
{
    int continuer = 1, i = 0;
    SDL_Event event;
                ...
                switch(event.key.keysym.sym)
                {
                    ...
                    case SDLK_c: // presser la touche C : algo de chiffrement
                        for(i = 0; i < 100 ;i++)
                        {
                            ...
                            strcat(nomNewImage,"images/barbara100/image");
                            itoa(j,buffer,10);
                            printf("j = %d = %s\n", j, buffer);
                            strcat(nomNewImage,buffer);
                            strcat(nomNewImage,".bmp");
                            image = IMG_Load(nomNewImage);
                            //SDL_BlitSurface(image, NULL, ecran, &positionImg);
                            //SDL_Flip(ecran);
                            // Tableau de bits de l'image
                            tabBits = recupTabBitsRGB(image, largeurImg, longueurImg, L, tabBits);
                            ...
                            retourImage(ecran, image, tabBits, L, largeurImg, longueurImg);
                            //nomNewImage = "images/barbaraAC2.bmp";
                            memset(nomNewImage, 0, sizeof(nomNewImage));
                            strcat(nomNewImage,"images/imageC");
                            itoa(j,buffer,10);
                            //printf("j = %d = %s\n", j, buffer);
                            j++;
                            strcat(nomNewImage,buffer);
...
/*******************************************************************/
int* recupTabBitsRGB(SDL_Surface* image, int largeurImg, int longueurImg, int L, int* tabBits)
{
    // Allocation de memoire dynamique pour utiliser le tas plutot que la pile qui est trop petite
    ...
    for(i=0;i < largeurImg;i++)
    {
        ...
        {
            pixel = obtenirPixel(image,i,j); // on recupere un pixel
            SDL_GetRGB(pixel, image->format, &r,&g,&b); // on recupere ses composantes RGB
            //printf("RGB1 : %d %d %d\n", r, g, b);
            /****/
...
/*******************************************************************/
void retourImage(SDL_Surface* ecran, SDL_Surface* image, int* tabBits, int L, int largeurImg, int longueurImg)
{
    SDL_Rect positionImg;
    positionImg.x = 0;
    positionImg.y = 0;
    ...
    for(i = 0; i < L ; i = i+8)
    {
        ...
        else if(k == 2)
        {
            b = decimal;
            k = 0;
            pixel = SDL_MapRGBA(image->format, r, g, b, 255); // reformer un pixel
            pixels[l] = pixel;
            l++;
        }
        else
    ...
    for(i = 0; i < largeurImg ;i++)
    {
        for(j = 0; j < longueurImg ;j++)
        {
            definirPixel(image,i,j,pixels[l]); // remplacer le pixel initial par le nouveau
            l++;
        }
    }
