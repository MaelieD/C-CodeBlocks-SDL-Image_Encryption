...
void pause(int, double, int*, SDL_Surface*, SDL_Surface*, int, int, int*, SDL_Rect, SDL_PixelFormat*); // garder la fenetre ouverte jusqu'a fermeture via l'event "click de souris sur la croix rouge"
...
int main(int argc, char *argv[])
{
    ...
    image = IMG_Load("images/imageC1.bmp");
    SDL_PixelFormat* fmt = image->format;
    if(fmt->BitsPerPixel == 8)
        L = 8*largeurImg*longueurImg;
    SDL_BlitSurface(image, NULL, ecran, &positionImg);
    SDL_Flip(ecran);
/******/

/******/
    pause(L, R, tabBits, ecran, image, largeurImg, longueurImg, K, positionImg, fmt);
...
/*******************************************************************/
void pause(int L, double R, int* tabBits, SDL_Surface* ecran, SDL_Surface* image, int largeurImg, int longueurImg, int* K, SDL_Rect positionImg, SDL_PixelFormat* fmt)
{
...
/*******************************************************************/
void definirPixel(SDL_Surface *image, int x, int y, Uint32 pixel, SDL_PixelFormat* fmt)
{
    int nbOctetsParPixel = fmt->BytesPerPixel;
